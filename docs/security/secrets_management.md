...
The recommended way to create K8s secrets is using the Helm deployment script:

```bash
# Create all K8s secrets from infra/secrets/keys/
./infra/helm/api-forge/scripts/apply-secrets.sh

# This creates in the api-forge-prod namespace:
# - app-secrets: Application secrets (session, CSRF, OIDC)
# - postgres-secrets: PostgreSQL passwords for all users
...
The deployment CLI handles secrets automatically:

```bash
# Deploy to Kubernetes - secrets are created automatically
uv run api-forge-cli deploy up k8s

# The CLI:
# 1. Checks for required secret files
# 2. Generates missing secrets if needed
# 3. Runs apply-secrets.sh automatically
# 4. Deploys Helm chart with all secrets
```
...
# Application secrets (includes OIDC client secrets)
...
kubectl create secret tls postgres-tls \
  --cert=infra/secrets/certs/postgres/server.crt \
...
kubectl create secret tls redis-tls \
  --cert=infra/secrets/certs/redis/server.crt \
  --key=infra/secrets/certs/redis/server.key \
  --namespace $NAMESPACE --dry-run=client -o yaml | kubectl apply -f -
```

#### How K8s Secrets Flow to Application (Helm Deployment)

The secret management flow in Helm-based Kubernetes deployment follows this pattern:

1. **Local Files** (`infra/secrets/keys/oidc_google_client_secret.txt`)
   - Contains the actual secret value (e.g., "your-google-client-secret")
   - Generated by `generate_secrets.sh` or manually created
   - Never committed to git (in `.gitignore`)

2. **K8s Secret** (created by `apply-secrets.sh` or CLI)
   - Stores the secret value in base64 in the cluster
   - Created from local files: `kubectl create secret ... --from-file=...`

3. **Volume Mount** (configured in Helm templates)
   - Template: `infra/helm/api-forge/templates/deployments/app.yaml`
   ```yaml
   volumeMounts:
     - name: app-secrets
       mountPath: /run/secrets/oidc_google_client_secret
       subPath: oidc_google_client_secret
       readOnly: true
   volumes:
     - name: app-secrets
       secret:
   ...
   ```
   - Mounts secret as a file at `/run/secrets/oidc_google_client_secret`

4. **Entrypoint Script** (`infra/docker/prod/scripts/universal-entrypoint.sh`)
   - Runs before application starts in container
   - Copies `/run/secrets/*` → `/app/keys/*` with secure permissions (0400)
   - Creates environment variables: `OIDC_GOOGLE_CLIENT_SECRET="your-google-client-secret"`
   - Environment variables take precedence over ConfigMap values

5. **Application** (reads secrets at startup)
   - Config uses env var substitution: `client_secret: "${OIDC_GOOGLE_CLIENT_SECRET}"`
   - Gets actual secret value from entrypoint-created env var
   - ConfigMap values (from `.env` file) are ignored

**Important**: OIDC client secrets should **NOT** be in `.env` files. They are:
- ✅ Stored in `infra/secrets/keys/oidc_*_client_secret.txt` (gitignored)
- ✅ Created as K8s secrets via `apply-secrets.sh` or CLI
- ✅ Mounted as files in pods at `/run/secrets/` by Helm templates
- ✅ Converted to env vars by entrypoint script
- ❌ NOT in `.env` or ConfigMap (those are just placeholders/comments)

This ensures secrets are:
- Never committed to source control
- Encrypted at rest in K8s (if cluster encryption enabled)
- Accessible only to authorized pods in the namespace
- Properly secured with file permissions (0400)
- Managed consistently across deployments via Helm

#### Helm-Specific Secret Management

**Verify secrets after Helm deployment:**
...
kubectl get secret app-secrets -n api-forge-prod \
  -o jsonpath='{.data.session_signing_secret}' | base64 -d

# Verify secret is mounted in pod
kubectl exec -n api-forge-prod deployment/app -- \
  ls -la /run/secrets/

# Check entrypoint processed secrets
kubectl exec -n api-forge-prod deployment/app -- \
  ls -la /app/keys/
```

**Update secrets after Helm deployment:**

```bash
# Update local secret files
vim infra/secrets/keys/session_signing_secret.txt

# Reapply secrets
./infra/helm/api-forge/scripts/apply-secrets.sh

# Redeploy with Helm (forces pod recreation via timestamp annotation)
uv run api-forge-cli deploy up k8s

# Or manually upgrade Helm release
