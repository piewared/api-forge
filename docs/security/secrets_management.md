...
The recommended way to create K8s secrets is using the provided script:
...
#### How K8s Secrets Flow to Application

The secret management flow in Kubernetes follows this pattern:

1. **Local Files** (`infra/secrets/keys/oidc_google_client_secret.txt`)
   - Contains the actual secret value (e.g., "GOOGLE!")
   - Generated by `generate_secrets.sh` or manually created
   - Never committed to git (in `.gitignore`)

2. **K8s Secret** (created by `apply-secrets.sh`)
   - Stores the secret value in base64 in the cluster
   - Created from local files: `kubectl create secret ... --from-file=...`

3. **Volume Mount** (configured in `k8s/base/deployments/app.yaml`)
   ```yaml
   volumeMounts:
     - name: app-secrets
...
4. **Entrypoint Script** (`infra/docker/prod/scripts/universal-entrypoint.sh`)
   ...
   - Creates environment variables: `OIDC_GOOGLE_CLIENT_SECRET="GOOGLE!"`
...
- ✅ Created as K8s secrets via `apply-secrets.sh`
- ✅ Mounted as files in pods at `/run/secrets/`
...
- Encrypted at rest in K8s (if enabled)
