"""Tests for the auth/user session services."""
...
class TestAuthSessionValidation:
    """Validation paths for AuthSessionService.validate_auth_session.

    These exercise the real storage and validation logic — no internal
    mocking. Bad inputs (wrong state / fingerprint) are passed in directly
    and the side effects (session deletion as a defense) are verified
    through storage state, not method-call assertions.
    """

    @pytest.mark.asyncio
    async def test_happy_path(self, auth_session_service: AuthSessionService) -> None:
        session_id = await auth_session_service.create_auth_session(
            pkce_verifier="verifier",
            state="state-123",
            nonce="nonce-456",
    ...
    async def test_unknown_session_returns_none(
        ...
        result = await auth_session_service.validate_auth_session(
            ...
            state="anything",
            client_fingerprint_hash="anything",
        )
        assert result is None

    @pytest.mark.asyncio
    async def test_state_mismatch_rejects_and_deletes_session(
        self, auth_session_service: AuthSessionService
    ) -> None:
        session_id = await auth_session_service.create_auth_session(
            pkce_verifier="verifier",
        ...
        assert result is None
        # Defense: session is wiped on state mismatch.
        assert await auth_session_service.get_auth_session(session_id) is None

    @pytest.mark.asyncio
    async def test_fingerprint_mismatch_rejects_and_deletes_session(
        self, auth_session_service: AuthSessionService
    ) -> None:
        session_id = await auth_session_service.create_auth_session(
            pkce_verifier="verifier",
            ...
            provider="keycloak",
            return_to="/dashboard",
            client_fingerprint_hash="real-fingerprint",
        )

        result = await auth_session_service.validate_auth_session(
            session_id=session_id,
            state="state",
            client_fingerprint_hash="attacker-fingerprint",
        )
