"""Tests for the compose database runtime adapter.

K8s-specific runtime tests live in
``tests/unit/cli/commands/k8s/test_db_runtime.py`` — that whole directory is
gated on ``include_k8s_deploy`` in ``copier.yml`` so the imports never reach
projects that strip the k8s subtree.
"""
...
@patch("src.cli.commands.db.runtime_compose.get_db_settings")
