...
- **Deployment rollback**: Use `uv run api-forge-cli deploy history` to view release history and `uv run api-forge-cli deploy rollback [REVISION]` to restore a previous deployment. Helm automatically rolls back on pod startup failure via `--rollback-on-failure`.
