...
@router.post("/start")
async def start(req: StartRequest, client_service: TemporalClientService = Depends(get_temporal_service)):
...
async def signal(workflow_id: str, signal_name: str, payload: dict, client_service: TemporalClientService = Depends(get_temporal_service)):
    client = await client_service.get_client()
...
async def read(workflow_id: str, client_service: TemporalClientService = Depends(get_temporal_service)):
    client = await client_service.get_client()
    h = client.get_workflow_handle(workflow_id)
