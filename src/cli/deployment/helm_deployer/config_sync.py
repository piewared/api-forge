...
class ConfigSynchronizer:
    ...
    def _generate_target_env_file(self) -> None:
        """Generate a K8s-specific .env with promoted, cleaned-up vars.

        Instead of copying the raw .env verbatim (which may contain
        Fly.io-specific URLs, DEVELOPMENT_* vars, and placeholders),
        this produces a clean file with only production-resolved values.
        """
        from src.cli.deployment.env_resolver import resolve_env_for_target
        from src.cli.shared.env_file import parse_env_file

        raw_env = (
            parse_env_file(self.paths.env_file) if self.paths.env_file.exists() else {}
        )
        resolved = resolve_env_for_target(raw_env, "production")

        for dest_dir in (
            self.paths.helm_files,
            self.paths.postgres_standalone_chart / "files",
        ):
            dest_dir.mkdir(parents=True, exist_ok=True)
            dest = dest_dir / ".env"
            lines = [f"{k}={v}" for k, v in sorted(resolved.items())]
            dest.write_text("\n".join(lines) + "\n")

        self.console.print("  [dim]✓ Generated target-specific .env[/dim]")

    def copy_config_files(self, progress_factory: type[Progress]) -> None:
        """Copy configuration files to Helm staging area.

        Generates a target-resolved .env (instead of raw copy), then copies
        config.yaml, PostgreSQL configs, Temporal scripts, and entrypoint
        scripts to infra/helm/api-forge/files/ and
        ...
        # Generate target-resolved .env instead of copying raw file
        self._generate_target_env_file()
    ...
    def _get_config_files_manifest(self) -> list[tuple[Path, str, str]]:
        ...
        return [
            # Core config files
            # .env is generated by _generate_target_env_file(), not copied raw.
