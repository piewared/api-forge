...
def update_env_vars(updates: dict[str, str]) -> None:
    """Update multiple keys in the project .env file in one pass.

    Updates existing keys or appends new ones if not found. Preserves
    comments and formatting of other lines.

    Note: ``src.app.runtime.config.config_loader`` exposes a similarly-named
    ``update_env_file(key, value)`` helper that takes a single key/value
    pair. This multi-key variant is renamed to avoid the name collision.
...
def save_password_to_secrets(password: str) -> None:
    ...
    existed = manager.exists("postgres_password")
    manager.write("postgres_password", password, backup=True)

    if existed:
        console.print(
            "[dim]Backed up existing password to postgres_password.txt.bak[/dim]"
        )
